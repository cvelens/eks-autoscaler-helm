            ...
            steps {
                script {
                    ...
                    try {
                        ...
                        echo "Helm validate succeeded. Updating GitHub status to success."
                        updateGitHubStatus('helm-validate', 'success', 'Helm Validate check passed', env.PR_COMMIT_SHA)
                    } catch (Exception e) {
                        echo "Helm validate failed: ${e.message}"
